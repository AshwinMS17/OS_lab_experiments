...
int getprocess(struct process pro[],int proc_size,int at)
{
    int index=-1;
    //int low=-1;
    for(int i=0;i<proc_size;i++)//pro[i].at<=at is changed
    {
    ...
    if(index==-1)
    {
        ...
        {
            if(pro[i].q==-1&&pro[i].status==0)//removed status=0
            {
                index=i;
                break;
            }
...
void main()
{
    ...
    for(int i=pro[0].at;count<proc_size;)//k is used to traverse through processes
    {
        ...
        if(pro[index].at<=i)//checks if next process in line has arrived or not
        {
            ...
            gantt[d].st=i;
            if(pro[index].rem_bt>tq)
            {
                printf("\n index %d \n",index);
                gantt[d].ct=i+tq;
                pro[index].rem_bt=pro[index].rem_bt-tq;
            }
            else
            {
